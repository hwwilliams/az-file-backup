...
class InvalidContacts(Exception):
    ...
    def __init__(self):
        Exception.__init__(self, "All available contacts have invalid phone numbers.")


class InvalidJsonContactsArray(Exception):
    # Custom exception raised when the contacts json array was not found
    def __init__(self):
        Exception.__init__(self, "Failed to find valid contacts json array.")


def get_contacts_data():
    contacts_json_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "settings", "contacts.json")
    )
    ...
    try:
        ...
        logger.error(f'contacts file not found: "{contacts_json_path}"')
        raise
    ...
    if "contacts" in contacts_dict:
        ...
        logger.error(f'Contacts json array not found from: "{contacts_json_path}".')
...
class Contacts:
    def __init__(self, twilio_client):
        ...
        (self.contacts_dict, self.contacts_json_path) = get_contacts_data()
    ...
    def lookup(self):
        ...
        for contact in self.contacts_dict:
            ...
            try:
                self.client.lookups.phone_numbers(contact_phone_number).fetch()
    ...
    def valid(self):
        if self.valid_contacts:
            if len(self.contacts_dict) == len(self.valid_contacts):
                ...
                logger.debug("1 contact with a valid phone number.")
            ...
            if len(self.invalid_contacts) == 1:
                logger.warning("1 contact with an invalid phone number.")
