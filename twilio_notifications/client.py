...
class EmptyEnvironmentVariables(Exception):
    ...
    def __init__(self):
        Exception.__init__(
            self,
            "One or more Twilio environment variables not found. (TWILIO_ACCOUNT_SID or TWILIO_AUTH_TOKEN or TWILIO_MSG_SERVICE_SID)",
        )
...
def get_client_credentials():
    ...
    if empty_env_vars:
        for env_var in empty_env_vars:
            logger.error(f"Twilio environment variable {env_var} not found.")
